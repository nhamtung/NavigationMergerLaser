...
{
	ROS_INFO("laserscan_multi_merger.cpp-62-reconfigureCallback()");
...
{
	ROS_INFO("laserscan_multi_merger.cpp-74-laserscan_topic_parser()");
	// LaserScan topics to subscribe
	ros::master::V_TopicInfo topics;
	ros::master::getTopics(topics);
	// ROS_INFO("laserscan_multi_merger.cpp-78-topics: %s", topics.c_str());
	...
	for(int i=0;i<tokens.size();++i)
	{
		...
		{
			ROS_INFO("laserscan_multi_merger.cpp-88-topics: %s", topics[j].name.c_str());
			if( (tokens[i].compare(topics[j].name) == 0) && (topics[j].datatype.compare("sensor_msgs/LaserScan") == 0) )
			{
				// ROS_INFO("laserscan_multi_merger.cpp-90-topics: %s", topics[j].name.c_str());
				tmp_input_topics.push_back(topics[j].name);
			}
		}
	}
	if(tmp_input_topics.size()<2){
		ROS_ERROR("laserscan_multi_merger.cpp-97-ERROR merger laserscan - %f laserscan received!", tmp_input_topics.size());
	}
	...
	if( (tmp_input_topics.size() != input_topics.size()) || !equal(tmp_input_topics.begin(),tmp_input_topics.end(),input_topics.begin()))
	{
		...
		if(input_topics.size() > 0)
		{
			...
			{
            	ROS_INFO("laserscan_multi_merger.cpp-117-Subscribing to topics: %s", input_topics[i].c_str());
            ...
            ROS_WARN("laserscan_multi_merger.cpp-124-Not subscribed to any topic.");
	}
}

LaserscanMerger::LaserscanMerger()
{
	ROS_INFO("laserscan_multi_merger.cpp-128-LaserscanMerger()");
	ros::NodeHandle nh("~");
	...
	ROS_INFO("laserscan_multi_merger.cpp-145-scan_destination_topic: %s", scan_destination_topic.c_str());
	laser_scan_publisher_ = node_.advertise<sensor_msgs::LaserScan> (scan_destination_topic.c_str(), 1, false);
	ROS_INFO("laserscan_multi_merger.cpp-147-laserscan_topics: %s", laserscan_topics.c_str());

}

void LaserscanMerger::scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan, std::string topic)
{
	// ROS_INFO("laserscan_multi_merger.cpp-151-scanCallback()");
	...
	try
	{
		...
		ROS_ERROR("laserscan_multi_merger.cpp-166-%s",ex.what());
		return;
	}
...
{
	// ROS_INFO("laserscan_multi_merger.cpp-202-pointcloud_to_laserscan()");
	...
	for(int i=0; i<points.cols(); i++)
	{
		...
		{
			ROS_INFO("laserscan_multi_merger.cpp-233-rejected for nan in point(%f, %f, %f)\n", x, y, z);
			continue;
		}
		...
		if (range_sq < range_min_sq_) {
			ROS_INFO("laserscan_multi_merger.cpp-240-rejected for range %f below minimum value %f. Point: (%f, %f, %f)", range_sq, range_min_sq_, x, y, z);
			continue;
		}
		...
		{
			// ROS_INFO("laserscan_multi_merger.cpp-247-rejected for angle %f not in range (%f, %f)\n", angle, output->angle_min, output->angle_max);
			continue;
		}
...
{
	ROS_INFO("laserscan_multi_merger.cpp-255-main()");
